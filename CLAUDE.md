...
- **`src/profiles.ts`** — Loads and validates SSH profiles from `profiles.json` at startup (not on first use). Validates required fields (`host`, `port`, `username`, `privateKeyPath`, `hostFingerprint`) and verifies the key file is readable before caching. Never exposes private keys or passphrases in responses.
- **`src/security.ts`** — Dangerous command detection (regex patterns) and `AuditLogger`. Log created with `0o600` permissions. Common secret patterns (password=, token=, Bearer) are redacted before writing.
...
- **Audit logging**: All operations logged to `audit.log` (0o600 permissions, non-blocking). Common secret patterns redacted. Requires external rotation (logrotate) for production.
...
- **Operation history & undo**: In-memory, max 100 entries. `previousContent` capped at 512KB (undo unavailable for larger files). History cleared on connect/disconnect, not persisted across restarts. Undo writes as UTF-8 — unreliable for binary files.
- **execCommand timeout**: All internal `ssh exec` calls race against `EXEC_TIMEOUT` (30s). Output truncated at 1MB. Use `cat -- path` and `rm -f -- path` to protect against filenames starting with `-`.
